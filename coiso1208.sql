...
-- Listar o nome do cliente, email e nome da cidade.
select  cliente.nome as "Nome do cliente", 
cliente.email as "Email do cliente" ,
cidade.nome as "Nome da Cidade" 
from cliente
inner join cidade on cliente.id = cidade.id;


-- 2 - Listar o nome do cliente, email, nome da cidade e nome do estado apenas das cidades do estado de SP
select cliente.nome as "Nome do cliente",cliente.email,cidade.nome as "Nome da cidade",estado.nome as "Nome do estado"
from cliente
inner join cidade on cliente.idcidade = cidade.id
inner join estado on cliente.idestado = estado.id
...
marca.nome as "Marca do produto"
from produto
...
select produto.nome as NomeProduto, marca.nome as NomeMarca, categoria.nome as NomeCategoria
from produto
inner join marca on produto.idMarca = marca.id
...
select pedido.id as 'Numero do Pedido',pedido.data_pedido as 'Data do Pedido',cliente.nome as 'Nome do Cliente',vendedor.nome as 'Nome do vendedor'
from pedido inner join cliente on pedido.idCliente = cliente.id
inner join vendedor on pedido.idVendedor = vendedor.id
...
select produto.nome as NomeProduto, produto.preco as PrecoProduto
from produto inner join itens_pedido on produto.id = itens_pedido.idProduto
inner join pedido on itens_pedido.idPedido = pedido.id
...
select *from cliente;
select pedido.id as 'Numero do pedido', cliente.nome as 'Nome do cliente'
from cliente
left join pedido on cliente.id = pedido.id;
 

-- 10 - Listar o nome, preço, nome da categoria e nome da marca do produto mais caro.
select produto.nome as "nome PROD",produto.preco as "preco PROD",categoria.nome as "nome CATE",marca.nome as "nome MARC"
from produto
inner join categoria on produto.idCategoria = categoria.id
inner join marca on produto.idmarca = marca.id
